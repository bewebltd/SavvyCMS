...
{
	public class GenTestAdminController : AdminBaseController {
		//
		// GET: /Admin/GenTest/
		...
		public ActionResult Index() {
			...
			dataList.PageLoad();
			return View("GenTestList", dataList);
		}

		public class ListViewModel : SavvyDataList<Models.GenTest> {
			...
			public ListViewModel() {
				...
				DefaultSortBy = new Models.GenTest().GetDefaultOrderBy();       // hint: to change the default order by for both admin and front end, override GetDefaultOrderBy in model 
			}
			public override Sql GetSql() {
				Sql sql = new Sql("SELECT * FROM GenTest where 1=1");
				if (SearchText != "") {
					sql.AddKeywordSearch(SearchText, new Models.GenTest().GetTextFieldNames(), true);  // search on all text fields - change this if slow
				}
		...
		/// GET: /Admin/GenTest/Create
		/// </summary>
		public ActionResult Create() {
			var data = new EditViewModel();
			Breadcrumbs.Current.AddBreadcrumb(3, "Add Gen Test");
		...
		/// GET: /Admin/GenTest/Edit/5
		/// </summary>
		public ActionResult Edit(int id) {
			var data = new EditViewModel();
			Breadcrumbs.Current.AddBreadcrumb(3, "Edit Gen Test");
			var record = Models.GenTest.LoadID(id);
			if(record == null)  {
				Web.ErrorMessage = "Gen Test not found (ID: "+id+")";
				return Redirect("~/Admin/GenTestAdmin");
			}
			CheckLock(record);
			data.GenTest = record;
			return View("GenTestEdit", data);
		...
		public class EditViewModel {
			public Models.GenTest GenTest;
		}
		
		/// <summary>
		/// Loads existing record and shows view form, with cancel button
		/// GET: /Admin/GenTest/View/5
		/// </summary>
		public ActionResult View(int id) {
			Breadcrumbs.Current.AddBreadcrumb(3, "View Gen Test");
			var record = Models.GenTest.LoadID(id);
			return View("GenTestView", record);
		}
		
		/// <summary>
		/// Loads existing record and shows export form, with cancel button
		/// GET: /Admin/GenTest/Export/5
		/// </summary>
		public ActionResult Export(int id) {
			var record = Models.GenTest.LoadID(id);
			Web.SetHeadersForExcel("detail "+record.GetName()+".xls");
			return View("GenTestExport", record);
		}

		/// <summary>
		/// Saves an existing record
		/// POST: /Admin/GenTest/Edit/5
		/// </summary>
		[HttpPost]
		public ActionResult Edit(int id, FormCollection collection) {
			return ProcessForm(Models.GenTest.LoadID(id));
		}

		protected ActionResult ProcessForm(Models.GenTest record) {
			try {
				if(record==null)record=new GenTest(); //check for deleted, or null, create a new one
			...
			if (!ModelState.IsValid) {
				...
				return View("GenTestEdit", new EditViewModel(){GenTest = record});
		...
		private void Save(Models.GenTest record, bool isNew) {
			...
			ActiveRecordLoader.ClearCache("GenTest");
		}
		...
		public ActionResult SaveSortOrder(string sortOrder) {
			...
			foreach (var id in ids) {
				Sql sql = new Sql("update GenTest set SortPosition=",pos*10,"where GenTestID=", id);
				sql.Execute();
				pos++;
			}
			ActiveRecordLoader.ClearCache("GenTest");
