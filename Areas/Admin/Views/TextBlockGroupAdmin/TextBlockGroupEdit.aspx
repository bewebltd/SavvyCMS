<%@ Page Title="Edit Text Block Group" Inherits="System.Web.Mvc.ViewPage<Models.TextBlockGroup>" Language="C#" MasterPageFile="~/Areas/Admin/admin-no-form.master" %>
  ...
  <%=Html.InfoMessage()%>
	...
	<form name="form" id="form" method="post" enctype="multipart/form-data" action="<%=Web.FullRawUrl %>">
		<table class="svyEdit" cellspacing="0">
			<tr>
				<th colspan="2">Text Block Group</th>
			</tr>
			<tr>
			...
			<tr>
				...
				<td class="field">
					<%if(record.IsNewRecord || Util.IsDevAccess()){ %>
						<%=new Forms.TextField(record.Fields.GroupName, true) %>
					<%}else{%>
						<%=new Forms.DisplayField(record.Fields.GroupName, true) %>
				 <%} %>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<td colspan="2" class="footer">
					...
					<div class="extra-footer-buttons">
						<%=Html.SavvyHelpText(new Beweb.HelpText("Text Block Group Edit")) %>
