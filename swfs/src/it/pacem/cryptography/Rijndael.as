...
package it.pacem.cryptography{
	...
	public class Rijndael {
		...
		*/
		...
		public function Rijndael(keySize:Number, blockSize:Number):void {
			...
			for (var j:Number = 0; j<this.Nb; j++) {
				...
				for (i = 0; i<4; i++) {
					if (dir == "encrypt") {
						...
						b[i] = this.mult_GF256(state[i][j], 0xE) ^ this.mult_GF256(state[(i+1)%4][j], 0xB) ^ this.mult_GF256(state[(i+2)%4][j], 0xD) ^ this.mult_GF256(state[(i+3)%4][j], 9);
					}
				}
				for (i = 0; i<4; i++) {
					state[i][j] = b[i];
